...
// Collectathon
defaultConfigInfo_t TFBaseInfo =
{
...
bool CGameConfigManager::GetDefaultGameBlock( KeyValues *pIn )
{
	...
	// Add TFBase to the list
	defaultConfigs.AddToTail( TFBaseInfo );
	...
	// Seal: Since this engine is only used for a select amount of games just force the game we're working on
	...
	GetRootGameDirectory( szPath, sizeof( szPath ), GetRootDirectory() );
	AddDefaultConfig( TFBaseInfo, pIn, GetRootDirectory(), szPath );
