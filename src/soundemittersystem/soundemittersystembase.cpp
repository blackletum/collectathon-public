...
#define MANIFEST_MOUNT_FILE			"scripts/game_sounds_manifest.mount"
#define GAME_SOUNDS_HEADER_BLOCK	"scripts/game_sounds_header.txt"

#define GAME_SOUNDS_MANIFEST( MANIFESTNAME, MANIFESTFILE, GAMENAME )										\
KeyValues *MANIFESTNAME = new KeyValues( MANIFESTFILE );													\
if ( filesystem->LoadKeyValues( *MANIFESTNAME, IFileSystem::TYPE_SOUNDEMITTER, MANIFESTFILE, GAMENAME ) )	\
{																											\
	AccumulateFileNameAndTimestampIntoChecksum( &crc, MANIFESTFILE );										\
	for ( KeyValues *sub = MANIFESTNAME->GetFirstSubKey(); sub != NULL; sub = sub->GetNextKey() )			\
	{																										\
		if ( !Q_stricmp( sub->GetName(), "precache_file" ) )												\
		{																									\
			AccumulateFileNameAndTimestampIntoChecksum( &crc, sub->GetString() );							\
			AddSoundsFromFile( sub->GetString(), false, false, false, GAMENAME );							\
			continue;																						\
		}																									\
		else if ( !Q_stricmp( sub->GetName(), "preload_file" ) )											\
		{																									\
			AccumulateFileNameAndTimestampIntoChecksum( &crc, sub->GetString() );							\
			AddSoundsFromFile( sub->GetString(), true, false, false, GAMENAME );							\
			continue;																						\
		}																									\
...
bool CSoundEmitterSystemBase::InternalModInit()
{
	...
	*/
	...
	GAME_SOUNDS_MANIFEST( tfmanifest, MANIFEST_FILE, "tfbase" );
	GAME_SOUNDS_MANIFEST( dodmanifest, MANIFEST_FILE, "dod" );
	GAME_SOUNDS_MANIFEST( sharedmanifest, MANIFEST_FILE, "shared" );
	GAME_SOUNDS_MANIFEST( csmanifest, MANIFEST_FILE, "cstrike" );
	GAME_SOUNDS_MANIFEST( hl2manifest, MANIFEST_FILE, "hl2_override" );
...
// Input  : *filename - 
//-----------------------------------------------------------------------------
void CSoundEmitterSystemBase::AddSoundsFromFile( const char *filename, bool bPreload, bool bIsOverride /*=false*/, bool bRefresh /*=false*/, const char *szSearchPath )
{
	CSoundScriptFile sf;
	...
	if ( filesystem->LoadKeyValues( *kv, IFileSystem::TYPE_SOUNDEMITTER, filename, szSearchPath ) )
	{
