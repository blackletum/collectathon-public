...
public:
	...
	virtual int GetWeaponID( void ) const		{ return DOD_WEAPON_BAR; }

	// weapon id for stats purposes
	virtual int GetStatsWeaponID( void )
	{
		if ( IsSemiAuto() )
			return DOD_WEAPON_BAR_SEMIAUTO;
		else 
...
LINK_ENTITY_TO_CLASS( dod_weapon_bar, CWeaponBAR );
PRECACHE_WEAPON_REGISTER( dod_weapon_bar );
