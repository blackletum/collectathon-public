...
public:
	...
	virtual int GetWeaponID( void ) const		{ return DOD_WEAPON_GARAND; }

	// weapon id for stats purposes
	virtual int GetStatsWeaponID( void )
	{
		if ( IsFullyZoomed() )
            return DOD_WEAPON_GARAND_ZOOMED;
		else 
...
LINK_ENTITY_TO_CLASS( dod_weapon_garand, CWeaponGarand );
PRECACHE_WEAPON_REGISTER( dod_weapon_garand );
...
#ifndef CLIENT_DLL
		CEffectData data;
		data.m_nHitBox = DOD_EJECTBRASS_GARANDCLIP;
