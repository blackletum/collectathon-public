...
float CDODSniperWeapon::GetWeaponAccuracy( float flPlayerSpeed )
{
	...
	float flSpread = m_pWeaponInfo->m_DODWeaponData.m_flAccuracy;
...
void CDODSniperWeapon::ZoomIn( void )
{
	...
	m_flTimeWeaponIdle = gpGlobals->curtime + m_pWeaponInfo->m_DODWeaponData.m_flTimeToIdleAfterFire;
...
bool CDODSniperWeapon::IsZoomingIn( void )
{
	...
	Vector CDODSniperWeapon::GetDesiredViewModelOffset( C_TFPlayer *pOwner )
	{
		...
		Vector vecResult = ( m_flZoomPercent * GetPCWpnData().m_DODWeaponData.m_vecIronSightOffset +
