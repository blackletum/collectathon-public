...
void CWeaponDODBase::Precache( void )
{
	...
	const CTFWeaponInfo &info = GetPCWpnData();

	int iWpnNameLen = Q_strlen( info.m_DODWeaponData.m_szReloadModel );
	...
	if( iWpnNameLen > 0 )
		m_iReloadModelIndex = CBaseEntity::PrecacheModel( info.m_DODWeaponData.m_szReloadModel );
}
...
#ifdef CLIENT_DLL
	void CWeaponDODBase::PlayWorldReloadSound( CTFPlayer *pPlayer )
	{
		Assert( pPlayer );
...
{
	//MeleeAttack( 60,  DOD_DMG_CUSTOM_MELEE_BUTTSTOCK | DOD_DMG_CUSTOM_MELEE_SECONDARYATTACK, 0.2f, 0.9f );
}

void CWeaponDODBase::Bayonet( void )
{
	//MeleeAttack( 60,  DOD_DMG_CUSTOM_MELEE_BAYONET | DOD_DMG_CUSTOM_MELEE_SECONDARYATTACK, 0.2f, 0.9f );
}

void CWeaponDODBase::Punch( void )
{
	MeleeAttack( 60, DOD_DMG_CUSTOM_MELEE_FIST | DOD_DMG_CUSTOM_MELEE_SECONDARYATTACK, 0.2f, 0.4f );
}
...
void CWeaponDODBase::Smack()
{
	...
	{
		if ( m_iSmackDamageType & DOD_DMG_CUSTOM_MELEE_STRONGATTACK )
...
#if 0 // SEALTODO I need to change this for backstabs, check if setting the damage to the smack damage type has any unintended affects
	if ( m_iSmackDamageType & DOD_DMG_CUSTOM_MELEE_SECONDARYATTACK )
		info.SetDamageCustom( DOD_DMG_CUSTOM_MELEE_SECONDARYATTACK );
#else
	info.SetDamageCustom( m_iSmackDamageType );
#endif
	...
	else if ( m_iSmackDamageType & DOD_DMG_CUSTOM_MELEE_EDGE )
	{
	...
	bool CWeaponDODBase::OnFireEvent( C_BaseViewModel *pViewModel, const Vector& origin, const QAngle& angles, int event, const char *options )
	{
		if( event == 5001 )
		{
			if ( ShouldDrawMuzzleFlash() )
			{
				...
				switch( GetPCWpnData().m_DODWeaponData.m_iMuzzleFlashType )
				{
	...
	{
		int iCriteria = GetPCWpnData().m_DODWeaponData.m_iAltWpnCriteria;
	...
	Vector CWeaponDODBase::GetDesiredViewModelOffset( C_TFPlayer *pOwner )
	{
		...
		return ( flPercent * GetPCWpnData().m_DODWeaponData.m_vecViewNormalOffset +
			( 1.0 - flPercent ) * GetPCWpnData().m_DODWeaponData.m_vecViewProneOffset );
	}
	...
	// Purpose: 
	//-----------------------------------------------------------------------------
	int	CWeaponDODBase::InternalDrawModel( int flags )
	{
