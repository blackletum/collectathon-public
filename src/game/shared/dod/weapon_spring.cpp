...
public:
	...
	virtual int GetWeaponID( void ) const		{ return DOD_WEAPON_SPRING; }

	// weapon id for stats purposes
	virtual int GetStatsWeaponID( void )
	{
		if ( IsFullyZoomed() )
			return DOD_WEAPON_SPRING_ZOOMED;
		else 
...
LINK_ENTITY_TO_CLASS( dod_weapon_spring, CWeaponSpring );
PRECACHE_WEAPON_REGISTER( dod_weapon_spring );
