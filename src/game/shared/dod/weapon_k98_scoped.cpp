...
public:
	...
	virtual int GetWeaponID( void ) const		{ return DOD_WEAPON_K98_SCOPED; }

	// weapon id for stats purposes
	virtual int GetStatsWeaponID( void )
	{
		if ( IsFullyZoomed() )
			return DOD_WEAPON_K98_SCOPED_ZOOMED;
		else 
			return DOD_WEAPON_K98_SCOPED;
	}
...
LINK_ENTITY_TO_CLASS( dod_weapon_k98_scoped, CWeaponK98Scoped );
PRECACHE_WEAPON_REGISTER( dod_weapon_k98_scoped );
