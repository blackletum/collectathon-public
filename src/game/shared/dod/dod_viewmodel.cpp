...
#ifdef CLIENT_DLL
ConVar cl_wpn_sway_interp_dod( "cl_wpn_sway_interp_dod", "0.1", FCVAR_CLIENTDLL );
ConVar cl_wpn_sway_scale_dod( "cl_wpn_sway_scale_dod", "2.6", FCVAR_CLIENTDLL );
#endif
...
void CDODViewModel::CalcViewModelLag( Vector& origin, QAngle& angles, QAngle& original_angles )
{
	...
	float flSwayScale = cl_wpn_sway_scale_dod.GetFloat();
	...
	m_vLagAngles = angles;
	m_LagAnglesHistory.NoteChanged( gpGlobals->curtime, cl_wpn_sway_interp_dod.GetFloat(), false );

	// Interpolate back 100ms.
	m_LagAnglesHistory.Interpolate( gpGlobals->curtime, cl_wpn_sway_interp_dod.GetFloat() );
...
#ifdef CLIENT_DLL
ConVar cl_gunlowerangle_dod( "cl_gunlowerangle_dod", "30", FCVAR_CLIENTDLL );
ConVar cl_gunlowerspeed_dod( "cl_gunlowerspeed_dod", "2", FCVAR_CLIENTDLL );
...
void CDODViewModel::CalcViewModelView( CBasePlayer *owner, const Vector& eyePosition, const QAngle& eyeAngles )
{
	...
	bool bLowered = pPlayer->IsDODWeaponLowered();

	QAngle vecLoweredAngles(0,0,0);

	m_vLoweredWeaponOffset.x = Approach( bLowered ? cl_gunlowerangle_dod.GetFloat() : 0, m_vLoweredWeaponOffset.x, cl_gunlowerspeed_dod.GetFloat() );
