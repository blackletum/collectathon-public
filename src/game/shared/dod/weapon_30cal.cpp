...
public:
	...
	virtual int GetWeaponID( void ) const		{ return DOD_WEAPON_30CAL; }

	// weapon id for stats purposes
	virtual int GetStatsWeaponID( void )
	{
		if ( !IsDeployed() )
			return DOD_WEAPON_30CAL_UNDEPLOYED;
		else 
...
LINK_ENTITY_TO_CLASS( dod_weapon_30cal, CWeapon30cal );
PRECACHE_WEAPON_REGISTER( dod_weapon_30cal );
...
#ifdef CLIENT_DLL
	C_TFPlayer *pPlayer = ToTFPlayer( GetPlayerOwner() );
#else
	CTFPlayer *pPlayer = ToTFPlayer( GetPlayerOwner() );
#endif
