...
public:
	...
	virtual int GetWeaponID( void ) const		{ return DOD_WEAPON_K98; }

	// weapon id for stats purposes
	virtual int GetStatsWeaponID( void )
	{
		if ( IsFullyZoomed() )
			return DOD_WEAPON_K98_ZOOMED;
		else 
			return DOD_WEAPON_K98;
	}
...
LINK_ENTITY_TO_CLASS( dod_weapon_k98, CWeaponK98 );
PRECACHE_WEAPON_REGISTER( dod_weapon_k98 );
