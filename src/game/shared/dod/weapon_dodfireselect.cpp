...
float CDODFireSelectWeapon::GetFireDelay( void )
{
	if ( IsSemiAuto() )
	{
		return m_pWeaponInfo->m_DODWeaponData.m_flSecondaryFireDelay;
	}
	else
	{
		return m_pWeaponInfo->m_DODWeaponData.m_flFireDelay;
	}	
}
...
float CDODFireSelectWeapon::GetWeaponAccuracy( float flPlayerSpeed )
{
	...
	if ( IsSemiAuto() )
		...
		flSpread = m_pWeaponInfo->m_DODWeaponData.m_flAccuracy;

	if( flPlayerSpeed > 45 )
		flSpread += m_pWeaponInfo->m_DODWeaponData.m_flAccuracyMovePenalty;
...
#ifdef CLIENT_DLL
	Vector CDODFireSelectWeapon::GetDesiredViewModelOffset( C_TFPlayer *pOwner )
	{
		...
		Vector offset = ( flPercent * GetPCWpnData().m_DODWeaponData.m_vecViewNormalOffset +
		...
		if ( prediction->InPrediction() )
		{
			return ( flLastPercent * offset +
				( 1.0 - flLastPercent ) * GetPCWpnData().m_DODWeaponData.m_vecViewProneOffset );
		}
		...
		return ( flZoomPercent * GetPCWpnData().m_DODWeaponData.m_vecIronSightOffset +
