...
CDODGameMovement::CDODGameMovement( CTFPlayer *pTFPlayer )
{
	m_pDODPlayer = pTFPlayer;
...
void CDODGameMovement::SetPlayerSpeed( void )
{
	...
	CWeaponDODBase *pWpn = dynamic_cast< CWeaponDODBase * >( m_pDODPlayer->GetActivePCWeapon() );
	if( pWpn )
	{
		if( pWpn->GetPCWpnData().m_DODWeaponData.m_WeaponType == WPN_TYPE_BAZOOKA )
		{
			CDODBaseRocketWeapon *pBazooka = (CDODBaseRocketWeapon *)pWpn;
			bBazookaDeployed = pBazooka->ShouldPlayerBeSlow();
		}

		if ( pWpn->GetPCWpnData().m_DODWeaponData.m_WeaponType == WPN_TYPE_SNIPER )
		{
...
void CDODGameMovement::CheckFalling( void )
{
	...
	if ( player->GetGroundEntity() != NULL && !IsDead() )
	{
		if ( player->m_Local.m_flFallVelocity >= 300 )
		{
			...
			player->EmitSound( filter, player->entindex(), "DODPlayer.JumpLanding" );
		}
...
void CDODGameMovement::ProcessMovement( CBasePlayer *pBasePlayer, CMoveData *pMove )
{
	...
	m_pDODPlayer = ToTFPlayer( pBasePlayer );
	Assert( m_pDODPlayer );
...
bool CDODGameMovement::CanAccelerate()
{
	...
	int curState = m_pDODPlayer->m_Shared.GetState();
	if ( curState == TF_STATE_ACTIVE )
	{
...
void CDODGameMovement::WalkMove( void )
{
	...
	if( bSprintButtonPressed && 
		...
		flSpeed > 80 )
	{
		m_pDODPlayer->m_Shared.SetSprinting( true );
	}
	else
	{
		m_pDODPlayer->m_Shared.SetSprinting( false );
	}
...
bool CDODGameMovement::CheckJumpButton( void )
{
	...
	filter.UsePredictionRules();
	m_pDODPlayer->EmitSound( filter, m_pDODPlayer->entindex(), "DODPlayer.Jump" );
