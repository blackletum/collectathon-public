...
void CDODBipodWeapon::Precache( void )
{
	...
	if( Q_strlen(info.m_DODWeaponData.m_szDeployedModel) > 0 )
	{
		Assert( info.m_DODWeaponData.m_iAltWpnCriteria & ALTWPN_CRITERIA_DEPLOYED );
		m_iDeployedModelIndex = CBaseEntity::PrecacheModel( info.m_DODWeaponData.m_szDeployedModel );
	}

	if( Q_strlen(info.m_DODWeaponData.m_szDeployedReloadModel) > 0 )
	{
		Assert( info.m_DODWeaponData.m_iAltWpnCriteria & ALTWPN_CRITERIA_DEPLOYED_RELOAD );
		m_iDeployedReloadModelIndex = CBaseEntity::PrecacheModel( info.m_DODWeaponData.m_szDeployedReloadModel );
	}

	if( Q_strlen(info.m_DODWeaponData.m_szProneDeployedReloadModel) > 0 )
	{
		Assert( info.m_DODWeaponData.m_iAltWpnCriteria & ALTWPN_CRITERIA_PRONE_DEPLOYED_RELOAD );
		m_iProneDeployedReloadModelIndex = CBaseEntity::PrecacheModel( info.m_DODWeaponData.m_szProneDeployedReloadModel );
	}
...
bool CDODBipodWeapon::TestDeployAngle( CTFPlayer *pPlayer, float *flDeployedHeight, CBaseEntity **pDeployedOn, QAngle angles )
{
...
float CDODBipodWeapon::GetWeaponAccuracy( float flPlayerSpeed )
{
	...
	if( IsDeployed() )
	{
		flSpread = m_pWeaponInfo->m_DODWeaponData.m_flSecondaryAccuracy;
	}
