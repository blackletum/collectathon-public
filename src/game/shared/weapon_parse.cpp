...
KeyValues* ReadEncryptedKVFile( IFileSystem *pFilesystem, const char *szFilenameWithoutExtension, const unsigned char *pICEKey, bool bForceReadEncryptedFile /*= false*/, const char *szExtension, const char *szSearchPath )
{
	...
	if ( szSearchPath != NULL )
		pSearchPath = szSearchPath;

	// Open the weapon data file, and abort if we can't
	KeyValues *pKV = new KeyValues( "WeaponDatafile" );

	Q_snprintf(szFullName,sizeof(szFullName), "%s%s", szFilenameWithoutExtension, szExtension);
