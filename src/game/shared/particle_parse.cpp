...
#define GAME_PARTICLE_MANIFEST( KVNAME, MANIFESTFILE, GAMENAME )																			\
	KeyValues *KVNAME = new KeyValues( MANIFESTFILE );																						\
	if ( KVNAME->LoadFromFile( filesystem, MANIFESTFILE, GAMENAME ) )																		\
	{																																		\
		for ( KeyValues *sub = KVNAME->GetFirstSubKey(); sub != NULL; sub = sub->GetNextKey() )												\
		{																																	\
			if ( !Q_stricmp( sub->GetName(), "file" ) )																						\
			{																																\
				char szParticle[256];																										\
				Q_snprintf( szParticle, sizeof( szParticle ), sub->GetString(), GAMENAME );													\
				list.AddToTail( szParticle );																								\
				continue;																													\
			}																																\
																																			\
...
void GetParticleManifest( CUtlVector<CUtlString>& list )
{
	...
	GAME_PARTICLE_MANIFEST( TFManifest, PARTICLES_MANIFEST_FILE, "tfbase" );
	GAME_PARTICLE_MANIFEST( DODManifest, PARTICLES_MANIFEST_FILE, "dod" );
	GAME_PARTICLE_MANIFEST( SharedManifest, PARTICLES_MANIFEST_FILE, "shared" );
	GAME_PARTICLE_MANIFEST( CSManifest, PARTICLES_MANIFEST_FILE, "cstrike" );
	GAME_PARTICLE_MANIFEST( HL2Manifest, PARTICLES_MANIFEST_FILE, "hl2_override" );
	GAME_PARTICLE_MANIFEST( HL1Manifest, PARTICLES_MANIFEST_FILE, "hl1" );
}
