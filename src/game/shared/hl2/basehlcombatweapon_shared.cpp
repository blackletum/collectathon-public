...
bool CBaseHLCombatWeapon::Deploy( void )
{
	...
	{
		// SEALTODO
		//CTFPlayer *pPlayer = assert_cast<CTFPlayer*>( GetOwner() );
...
float	g_HL2lateralBob;
float	g_HL2verticalBob;
...
float CBaseHLCombatWeapon::CalcViewmodelBob( void )
{
	...
	g_HL2verticalBob = speed*0.005f;
	g_HL2verticalBob = g_HL2verticalBob*0.3 + g_HL2verticalBob*0.7*sin(cycle);

	g_HL2verticalBob = clamp( g_HL2verticalBob, -7.0f, 4.0f );
	...
	g_HL2lateralBob = speed*0.005f;
	g_HL2lateralBob = g_HL2lateralBob*0.3 + g_HL2lateralBob*0.7*sin(cycle);
	g_HL2lateralBob = clamp( g_HL2lateralBob, -7.0f, 4.0f );
...
void CBaseHLCombatWeapon::AddViewmodelBob( CBaseViewModel *viewmodel, Vector &origin, QAngle &angles )
{
	...
	VectorMA( origin, g_HL2verticalBob * 0.1f, forward, origin );
	
	// Z bob a bit more
	origin[2] += g_HL2verticalBob * 0.1f;
	
	// bob the angles
	angles[ ROLL ]	+= g_HL2verticalBob * 0.5f;
	angles[ PITCH ]	-= g_HL2verticalBob * 0.4f;

	angles[ YAW ]	-= g_HL2lateralBob  * 0.3f;

	VectorMA( origin, g_HL2lateralBob * 0.8f, right, origin );
}
