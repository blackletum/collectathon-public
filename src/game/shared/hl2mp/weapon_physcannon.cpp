...
#define hl2_normspeed 190.0f
#define hl2_walkspeed 150.0f
#endif
...
void CPlayerPickupController::Init( CBasePlayer *pPlayer, CBaseEntity *pObject )
{
	...
	CTFPlayer *pOwner = (CTFPlayer*)ToBasePlayer( pPlayer );
	if ( pOwner )
	{
		// SEALTODO
...
void CPlayerPickupController::Shutdown( bool bThrown )
{
	...
	if ( m_pPlayer )
	{
		CTFPlayer *pOwner = (CTFPlayer*)ToBasePlayer( m_pPlayer );
		if ( pOwner )
		{
			// SEALTODO
...
public:
	...
	virtual int GetWeaponID( void ) const		{ return HL2_WEAPON_PHYSCANNON; }
...
LINK_ENTITY_TO_CLASS( hl2_weapon_physcannon, CWeaponPhysCannon );
PRECACHE_WEAPON_REGISTER( hl2_weapon_physcannon );
...
void CWeaponPhysCannon::DetachObject( bool playSound, bool wasLaunched )
{
	...
	if( pOwner != NULL )
	{
		// SEALTODO
		//pOwner->EnableSprint( true );
		pOwner->SetMaxSpeed( hl2_normspeed );
	}
