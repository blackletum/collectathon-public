...
{
	{ CS_WEAPON_P228,				"p228" },
	{ CS_WEAPON_GLOCK,				"glock" },
	{ CS_WEAPON_SCOUT,				"scout" },
	{ CS_WEAPON_XM1014,				"xm1014" },
	{ CS_WEAPON_MAC10,				"mac10" },
	{ CS_WEAPON_AUG,				"aug" },
	{ CS_WEAPON_ELITE,				"elite" },
	{ CS_WEAPON_FIVESEVEN,			"fiveseven" },
	{ CS_WEAPON_UMP45,				"ump45" },
	{ CS_WEAPON_SG550,				"sg550" },
	{ CS_WEAPON_GALIL,				"galil" },
	{ CS_WEAPON_FAMAS,				"famas" },
	{ CS_WEAPON_USP,				"usp" },
	{ CS_WEAPON_AWP,				"awp" },
	{ CS_WEAPON_MP5NAVY,			"mp5navy" },
	{ CS_WEAPON_M249,				"m249" },
	{ CS_WEAPON_M3,					"m3" },
	{ CS_WEAPON_M4A1,				"m4a1" },
	{ CS_WEAPON_TMP,				"tmp" },
	{ CS_WEAPON_G3SG1,				"g3sg1" },
	{ CS_WEAPON_DEAGLE,				"deagle" },
	{ CS_WEAPON_SG552,				"sg552" },
	{ CS_WEAPON_AK47,				"ak47" },
	{ CS_WEAPON_P90,				"p90" },

	{ CS_WEAPON_KNIFE,				"knife" },
	{ CS_WEAPON_C4,					"c4" },
	{ CS_WEAPON_FLASHBANG,			"flashbang" },
	{ CS_WEAPON_SMOKEGRENADE,		"smokegrenade" },
	{ CS_WEAPON_SMOKEGRENADE,		"sgren" },
	{ CS_WEAPON_HEGRENADE,			"hegrenade" },
	{ CS_WEAPON_HEGRENADE,			"hegren" },

	// not sure any of these are needed
	{ CS_WEAPON_SHIELDGUN,			"shield" },
	{ CS_WEAPON_SHIELDGUN,			"shieldgun" },
	{ CS_WEAPON_KEVLAR,				"kevlar" },
	{ CS_WEAPON_ASSAULTSUIT,		"assaultsuit" },
	{ CS_WEAPON_NVG,				"nightvision" },
	{ CS_WEAPON_NVG,				"nvg" },
...
{
	const CTFWeaponInfo* pWeaponInfo = GetCSWeaponInfo( id );
	if ( pWeaponInfo )
	{
...
{
	if ( !Q_strcmp( g_aAmmoNames[BULLET_PLAYER_762MM], ammoname ) )
		return CS_SHELL_762NATO;

	if ( !Q_strcmp( g_aAmmoNames[BULLET_PLAYER_556MM], ammoname ) )
		return CS_SHELL_556;

	if ( !Q_strcmp( g_aAmmoNames[BULLET_PLAYER_338MAG], ammoname ) )
		return CS_SHELL_338MAG;

	if ( !Q_strcmp( g_aAmmoNames[BULLET_PLAYER_BUCKSHOT], ammoname ) )
		return CS_SHELL_12GAUGE;

	if ( !Q_strcmp( g_aAmmoNames[BULLET_PLAYER_57MM], ammoname ) )
		return CS_SHELL_57;
...
CWeaponCSBase::CWeaponCSBase()
{
	...
	m_weaponMode = TF_WEAPON_PRIMARY_MODE;
}
...
{
	return GetPCWpnData().m_CSWeaponData.m_WeaponType == WEAPONTYPE_PISTOL;
}


bool CWeaponCSBase::IsFullAuto() const
{
	return GetPCWpnData().m_CSWeaponData.m_bFullAuto;
}
...
void CWeaponCSBase::AddToPriorOwnerList(CTFPlayer* pPlayer)
{
...
bool CWeaponCSBase::IsAPriorOwner(CTFPlayer* pPlayer)
{
...
void CWeaponCSBase::ItemPostFrame()
{
	...
	else if ((pPlayer->m_nButtons & IN_ATTACK) && (m_flNextPrimaryAttack <= gpGlobals->curtime ))
	{
		// SEALTODO
#if 0
		if ( TFGameRules()->IsFreezePeriod() )	// Can't shoot during the freeze period
			return;
		...
		if ( pPlayer->m_Shared.GetState() != TF_STATE_ACTIVE )
			return;
		...
		if ( GetPCWpnData().m_CSWeaponData.m_WeaponType != WEAPONTYPE_GRENADE )
		{
			...
			if( event )
			{
				...
				if ( strncmp( weaponName, "cs_weapon_", 10 ) == 0 )
				{
					weaponName += 10;
				}
...
float CWeaponCSBase::GetInaccuracy() const
{
	...
	const CTFWeaponInfo& weaponInfo = GetPCWpnData();

	float fMaxSpeed = GetMaxSpeed();
	if ( fMaxSpeed == 0.0f )
		fMaxSpeed = GetPCWpnData().m_CSWeaponData.m_flMaxSpeed;
	...
	return m_fAccuracyPenalty + 
		...
		0.0f, weaponInfo.m_CSWeaponData.m_fInaccuracyMove[m_weaponMode]);
}
...
float CWeaponCSBase::GetSpread() const
{
	...
	return GetPCWpnData().m_CSWeaponData.m_fSpread[m_weaponMode];
}
...
float CWeaponCSBase::GetMaxSpeed() const
{
	...
	float flRet = GetPCWpnData().m_CSWeaponData.m_flMaxSpeed;
	Assert( flRet > 1 );
	return flRet;
}
...
#ifdef CS_SHIELD_ENABLED
	if ( GetPCWpnData().m_bCanUseWithShield )
	{
		 PrecacheModel( GetPCWpnData().m_szShieldViewModel );
	}
#endif
...
#if 0
	if ( pPlayer->HasShield() && GetPCWpnData().m_CSWeaponData.m_bCanUseWithShield == false )
		 return false;
#endif
...
// SEALTODO
	...
	void CWeaponCSBase::DrawCrosshair()
	{
		...
		CTFPlayer* pPlayer = (CTFPlayer*)C_BasePlayer::GetLocalPlayer();

		if ( !pPlayer )
			return;

		// localplayer must be owner if not in Spec mode
		Assert( (pPlayer == GetTFPlayerOwner()) || ( pPlayer->GetObserverMode()==OBS_MODE_IN_EYE) );
		...
		bool bCrosshairVisible = crosshair.GetBool() && GetPCWpnData().m_CSWeaponData.m_WeaponType != WEAPONTYPE_SNIPER_RIFLE;
		...
		default:
			{
				// static crosshair
				float fSpread = (GetPCWpnData().m_CSWeaponData.m_fSpread[m_weaponMode] + GetPCWpnData().m_CSWeaponData.m_fInaccuracyStand[m_weaponMode]) * 320.0f / tanf(fHalfFov);
		...
		case 3:
			{
				float fCrosshairDistanceGoal = GetPCWpnData().m_CSWeaponData.m_iCrosshairMinDistance; // The minimum distance the crosshair can achieve...
				...
				int iDeltaDistance = GetPCWpnData().m_CSWeaponData.m_iCrosshairDeltaDistance; // Amount by which the crosshair expands when shooting (per frame)
	...
	bool CWeaponCSBase::OnFireEvent( C_BaseViewModel *pViewModel, const Vector& origin, const QAngle& angles, int event, const char *options )
	{
		if( event == 5001 )
		{
			...
			data.m_flScale = GetPCWpnData().m_CSWeaponData.m_flMuzzleScale;
	...
	{
		return GetPCWpnData().m_CSWeaponData.m_iMuzzleFlashStyle;
	}
	...
	{
		CTFPlayer *pPlayer = dynamic_cast< CTFPlayer* >( GetOwner() );
		if ( !pPlayer )
			return;
...
bool CWeaponCSBase::DefaultPistolReload()
{
	...
	if ( !DefaultReload( GetPCWpnData().iDefaultClip1, 0, ACT_VM_RELOAD ) )
		return false;
...
void CWeaponCSBase::UpdateAccuracyPenalty()
{
	...
	{
		fNewPenalty += weaponInfo.m_CSWeaponData.m_fInaccuracyStand[m_weaponMode] + weaponInfo.m_CSWeaponData.m_fInaccuracyLadder[m_weaponMode];
	}
	// in the air?
...
// 	}
	...
	{
		fNewPenalty += weaponInfo.m_CSWeaponData.m_fInaccuracyCrouch[m_weaponMode];
	}
	else
	{
		fNewPenalty += weaponInfo.m_CSWeaponData.m_fInaccuracyStand[m_weaponMode];
	}

	if ( m_bInReload )
	{
 		fNewPenalty += weaponInfo.m_CSWeaponData.m_fInaccuracyReload;
	}
	...
	else
	{
		...
		if ( pPlayer->GetMoveType() == MOVETYPE_LADDER )
		{
			fDecayFactor = logf(10.0f) / weaponInfo.m_CSWeaponData.m_fRecoveryTimeStand;
		}
		else if ( !FBitSet(pPlayer->GetFlags(), FL_ONGROUND) )	// in air
		{
			...
			fDecayFactor = logf(10.0f) / (weaponInfo.m_CSWeaponData.m_fRecoveryTimeCrouch * 3.0f);
		}
		else if ( FBitSet(pPlayer->GetFlags(), FL_DUCKING) )
		{
			fDecayFactor = logf(10.0f) / weaponInfo.m_CSWeaponData.m_fRecoveryTimeCrouch;
		}
		else
		{
			fDecayFactor = logf(10.0f) / weaponInfo.m_CSWeaponData.m_fRecoveryTimeStand;
...
{
	m_fAccuracyPenalty += GetPCWpnData().m_CSWeaponData.m_fInaccuracyJump[m_weaponMode] * fImpulse / kJumpVelocity;
}

void CWeaponCSBase::OnLand( float fVelocity )
{
	float fPenalty = GetPCWpnData().m_CSWeaponData.m_fInaccuracyLand[m_weaponMode] * fVelocity / kJumpVelocity;
