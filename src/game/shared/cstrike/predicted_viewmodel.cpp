...
#ifdef CLIENT_DLL
ConVar cl_wpn_sway_interp_cs( "cl_wpn_sway_interp_cs", "0.1", FCVAR_CLIENTDLL );
ConVar cl_wpn_sway_scale_cs( "cl_wpn_sway_scale_cs", "1.0", FCVAR_CLIENTDLL|FCVAR_CHEAT );
#endif
...
void CPredictedViewModel::CalcViewModelLag( Vector& origin, QAngle& angles, QAngle& original_angles )
{
		...
		m_vLagAngles = angles;
		m_LagAnglesHistory.NoteChanged( gpGlobals->curtime, cl_wpn_sway_interp_cs.GetFloat(), false );
		
		// Interpolate back 100ms.
		m_LagAnglesHistory.Interpolate( gpGlobals->curtime, cl_wpn_sway_interp_cs.GetFloat() );
		...
		vForwardDiff *= cl_wpn_sway_scale_cs.GetFloat();
