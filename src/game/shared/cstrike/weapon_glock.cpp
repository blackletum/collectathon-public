...
public:
	...
	virtual int GetWeaponID( void ) const		{ return CS_WEAPON_GLOCK; }
...
LINK_ENTITY_TO_CLASS( cs_weapon_glock, CWeaponGlock );
PRECACHE_WEAPON_REGISTER( cs_weapon_glock );
...
void CWeaponGlock::PrimaryAttack()
{
	...
	float flCycleTime = m_bBurstMode ? 0.5f : GetPCWpnData().m_CSWeaponData.m_flCycleTime;
	...
	// update accuracy
	m_fAccuracyPenalty += GetPCWpnData().m_CSWeaponData.m_fInaccuracyImpulseFire[m_weaponMode];
...
void CWeaponGlock::FireRemaining( float fSpread )
{
	...
	// update accuracy
	m_fAccuracyPenalty += GetPCWpnData().m_CSWeaponData.m_fInaccuracyImpulseFire[TF_WEAPON_SECONDARY_MODE];
}
