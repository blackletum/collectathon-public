...
{
#ifdef GAME_DLL
	const char* pszPlayerDisplayModel = NULL;
	if (GetAttributeContainer() && GetAttributeContainer()->GetItem())
	{
		for (int i = TF_TEAM_RED; i < TF_TEAM_COUNT; i++)
		{
			for (int j = TF_CLASS_SCOUT; j < TF_CLASS_COUNT; j++)
			{
				pszPlayerDisplayModel = GetAttributeContainer()->GetItem()->GetPlayerDisplayModel(j, i);
				if (pszPlayerDisplayModel != NULL)
				{
					if (!engine->IsModelPrecached(pszPlayerDisplayModel))
						PrecacheModel(pszPlayerDisplayModel);
				}
			}
		}
	}
...
#ifdef CLIENT_DLL
		CEconItemView* pView = pItem->GetAttributeContainer()->GetItem();
		if (pView)
		{
			if (cl_disable_cosmetics.GetBool())
			{
				if (pView->ShouldHideItemClientside())
					continue;
			}
		}
#endif
