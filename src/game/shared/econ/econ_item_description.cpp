...
)
{
	...
	// If we don't have a localization token for this attribute desc, assume the actual string as token.
	if (!loc_String)
	{
		pLocalizationProvider->ConvertUTF8ToLocchar(pAttribDef->GetDescriptionString(), &m_loc_sValue);
		pLocalizationProvider->ConvertUTF8ToLocchar(pAttribDef->GetDescriptionString(), &m_loc_sShortValue);
		return;
	}
