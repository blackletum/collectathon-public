...
#ifdef TF_CLIENT_DLL
extern ConVar cl_disable_cosmetics;
extern ConVar cl_flipviewmodels;
ConVar cl_disable_unusuals("cl_disable_unusuals", "0", FCVAR_ARCHIVE | FCVAR_CLIENTDLL, "Disable unusuals");
#ifdef STAGING_ONLY
...
Activity CEconEntity::TranslateViewmodelHandActivity( Activity actBase )
{
	...
	if ( pItem && pItem->IsValid() )
	{
		...
		if ( pStaticData && pStaticData->GetAttachToHandsMode() == ATTACH_TO_HANDS_WEAPON_TO_HANDS )
		{
...
void CEconEntity::UpdateModelToClass( void )
{
	...
	if ( pItem->GetStaticData()->GetAttachToHandsMode() == ATTACH_TO_HANDS_WEAPON_TO_HANDS )
	{
...
void CEconEntity::UpdateAttachmentModels( void )
{
	...
	if (pTFPlayer)
	{
		...
		if (pTFPlayer)
			CALL_ATTRIB_HOOK_FLOAT_ON_OTHER(pTFPlayer, iHandModelIndex, wrench_builds_minisentry);
	}
	...
	// Update the state of attachment models for this item
 	bool bItemNeedsAttachment = pItemDef && (pItemDef->GetAttachToHandsMode() != ATTACH_TO_HANDS_NO || pItemDef->ShouldAttachToHandsVMOnly());
		...
		if ( bShouldShowAttachment && AttachmentModelsShouldBeVisible() )
		{
			if ( !m_hViewmodelAttachment )
			{
				...
				if ( vm )
				{
					...
					{
						if (pEnt->InitializeAsClientEntity(pTFPlayer->GetPlayerClass()->GetHandModelName(iHandModelIndex), RENDER_GROUP_VIEW_MODEL_OPAQUE) == false)
							return;
					}
...
#ifdef TF_CLIENT_DLL
	if (cl_disable_cosmetics.GetBool() || cl_disable_unusuals.GetBool())
	{
		C_TFPlayer* pOwner = ToTFPlayer(GetOwnerEntity());
		bool bMVMBot = TFGameRules()->IsMannVsMachineMode() && pOwner && (pOwner->GetFlags() & FL_FAKECLIENT);
		if (!bMVMBot)
		{
			SetParticleSystemsVisible(PARTICLE_SYSTEM_STATE_NOT_VISIBLE);
			return;
...
int	CEconEntity::DrawOverriddenViewmodel( C_BaseViewModel *pViewmodel, int flags )
{
	...
	CEconItemView *pItem = GetAttributeContainer()->GetItem();
	bool bAttachesToHands = ( pItem->IsValid() && (pItem->GetStaticData()->GetAttachToHandsMode() != ATTACH_TO_HANDS_NO ||
		pItem->GetStaticData()->ShouldAttachToHandsVMOnly()));
...
#ifdef CLIENT_DLL
bool CEconEntity::WantsToOverrideViewmodelAttachments(void)
{
	CEconItemView* pItem = GetAttributeContainer()->GetItem();
	if (pItem && pItem->IsValid())
	{
		GameItemDefinition_t* pStaticData = pItem->GetStaticData();
		if (pStaticData && pStaticData->GetAttachToHandsMode() == ATTACH_TO_HANDS_HANDS_TO_WEAPON)
			return false;
