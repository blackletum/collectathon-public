...
{
	ATTACH_TO_HANDS_NO,
	ATTACH_TO_HANDS_WEAPON_TO_HANDS,
	ATTACH_TO_HANDS_HANDS_TO_WEAPON
};
...
public:
	...
	int			GetAttachToHandsMode(void) const { return m_nAttachToHandsMode; }
	...
	bool		HasExtraWearable( void ) const					{ return m_pszWorldExtraWearableModel != NULL; }
...
public:
	...
	typedef CUtlMap<const char*, CUtlVector<CEconItemDefinition*>*, int> PrefabItemsMap_t;
...
protected:
	...
	virtual bool BInitNativeScript(KeyValues* pKVRawDefinition, CUtlVector<CUtlString>* pVecErrors = NULL);
#ifdef TF_CLIENT_DLL
...
private:
	...
	// Mounting
	//
	bool BInitMountedScripts(CUtlVector<CUtlString>* pVecErrors);
	bool BMountExtraScript(const char* pMountFileName, const char* pathId, CUtlVector<CUtlString>* pVecErrors);
	bool BInitMountedScript(KeyValues* pKVCommunityMarketRemaps, CUtlVector<CUtlString>* pVecErrors);

	void RegisterItemDefinitionPrefabs(CEconItemDefinition* pItemDef);
	CUtlVector<CEconItemDefinition*>* GetItemListForDefinitionPrefab(const char* pszPrefabName);
	...
	// Contains information about what items are using a specific prefab.
	PrefabItemsMap_t									m_mapPrefabItems;
