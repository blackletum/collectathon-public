...
bool CEconItemView::ShouldHideItemClientside(void)
{
	if (GetItemDefinition() && GetStaticData())
	{
		if (!GetStaticData()->IsAWearable())
			return false;

		if (!GetItemDefinition()->IsActingAsAWearable() && !GetItemDefinition()->HasExtraWearable() &&
			GetItemDefinition()->GetDefaultLoadoutSlot() != LOADOUT_POSITION_ACTION)
			return true;
	}
