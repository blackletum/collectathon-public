...
bool CEconItemDefinition::BInitFromTestItemKVs( int iNewDefIndex, KeyValues *pKVItem, CUtlVector<CUtlString>* pVecErrors )
{
	...
	if ( !bTestingExistingItem )
	{
		...
		m_nAttachToHandsMode = pKVItem->GetInt("attach_to_hands");
...
#define daylight _daylight
	...
	m_nAttachToHandsMode = m_pKVItem->GetInt("attach_to_hands");
...
,	m_mapPrefabItems( CaselessStringLessThan )
...
void CEconItemSchema::Reset( void )
{
	...
	FOR_EACH_MAP_FAST(m_mapPrefabItems, i)
	{
		delete m_mapPrefabItems[i];
	}
	m_mapPrefabItems.Purge();
...
{
	// Only do this for native item schema.
	...
	//
	// We don't want to use schema substeps here because even if native script fails,
	// we still want to read mount files.
	// 

	BInitNativeScript(pKVRawDefinition, pVecErrors);
	BInitMountedScripts(pVecErrors);

	return SCHEMA_INIT_SUCCESS();
}

bool CEconItemSchema::BInitNativeScript(KeyValues* pKVRawDefinition, CUtlVector<CUtlString>* pVecErrors)
{
...
bool CEconItemSchema::BInitDefinitionPrefabs( KeyValues *pKVPrefabs, CUtlVector<CUtlString> *pVecErrors )
{
	FOR_EACH_TRUE_SUBKEY( pKVPrefabs, pKVPrefab )
	{
		...
		DevMsg("Declare Prefab \"%s\"\n", pszPrefabName);

		int nIndex = m_mapDefinitionPrefabs.Find( pszPrefabName );
		if (m_mapDefinitionPrefabs.IsValidIndex(nIndex))
			m_mapDefinitionPrefabs.RemoveAt(nIndex);

		m_mapDefinitionPrefabs.Insert( pszPrefabName, pKVPrefab->MakeCopy() );

		int nListIndex = m_mapPrefabItems.Find(pszPrefabName);
		if (m_mapPrefabItems.IsValidIndex(nListIndex))
		{
			CUtlVector<CEconItemDefinition*>* itemList = m_mapPrefabItems[nListIndex];
			DevMsg("Reloading %d items\n", itemList->Count());

			FOR_EACH_VEC(*itemList, i)
			{
				CEconItemDefinition* pItemDef = itemList->Element(i);
				if (!pItemDef)
					continue;

				KeyValues* pRawKV = pItemDef->m_pRawKVItem;
				DevMsg("- Reloading \"%s\"\n", pItemDef->GetDefinitionName());
				pItemDef->BInitFromKV(pRawKV);
				pRawKV->deleteThis();
			}
		}
		else
		{
			DevMsg("Created list for \"%s\"\n", pszPrefabName);
			m_mapPrefabItems.Insert(pszPrefabName, new CUtlVector<CEconItemDefinition*>());
		}
	}
...
CUtlVector<CEconItemDefinition*> *CEconItemSchema::GetItemListForDefinitionPrefab(const char* pszPrefabName)
{
	int nIndex = m_mapPrefabItems.Find(pszPrefabName);
	if (m_mapPrefabItems.IsValidIndex(nIndex))
		return m_mapPrefabItems[nIndex];
	...
	if ( NULL != pKVItems )
	{
		FOR_EACH_TRUE_SUBKEY( pKVItems, pKVItem )
		{
			...
			else
			{
				...
				// If we get a duplicate item definition, override it with what we want to put.
				if (m_mapItems.IsValidIndex(nMapIndex))
				{
					m_mapItems.RemoveAt(nMapIndex);

					CEconItemDefinition* pDeleteItemDef = m_mapItems.Element(nMapIndex);
					if (pDeleteItemDef)
					{
						m_vecBundles.FindAndRemove(pDeleteItemDef);
						m_vecItemCollectionCrates.FindAndRemove(pDeleteItemDef->GetDefinitionIndex());

						DevMsg("Overriding Item Definition %d (removing: \"%s\")\n", nMapIndex, pDeleteItemDef->GetItemDefinitionName());
						delete pDeleteItemDef;
					}
				}
				...
				nMapIndex = m_mapItems.InsertOrReplace( nItemIndex, pItemDef );
				m_mapItemsSorted.InsertOrReplace( nItemIndex, pItemDef );
				SCHEMA_INIT_SUBSTEP( m_mapItems[nMapIndex]->BInitFromKV( pKVItem, pVecErrors ) );

				// Cache off Tools references
				if ( pItemDef->IsTool() )
				{
					m_mapToolsItems.InsertOrReplace( nItemIndex, pItemDef );
				}

				if ( pItemDef->IsBaseItem() )
				{
					m_mapBaseItems.InsertOrReplace( nItemIndex, pItemDef );
				}
				...
				// register that this item is using this prefab
				RegisterItemDefinitionPrefabs(pItemDef);
			}
		}
	}
...
bool CEconItemSchema::BInitMountedScripts(CUtlVector<CUtlString>* pVecErrors)
{
	// Load the items_mount.txt
	SCHEMA_INIT_SUBSTEP(BMountExtraScript("scripts/items/items_game.mount", "MOD", pVecErrors));

	return SCHEMA_INIT_SUCCESS();
}

bool CEconItemSchema::BMountExtraScript(const char* pMountFileName, const char* pathID, CUtlVector<CUtlString>* pVecErrors)
{
	// Read the raw data
	CUtlBuffer bufRawData;
	bool bReadFileOK = g_pFullFileSystem->ReadFile(pMountFileName, pathID, bufRawData);
	SCHEMA_INIT_CHECK(bReadFileOK, "Cannot mount file '%s'", pMountFileName);

	// Wrap it with a text buffer reader
	CUtlBuffer bufText(bufRawData.Base(), bufRawData.TellPut(), CUtlBuffer::READ_ONLY | CUtlBuffer::TEXT_BUFFER);

	KeyValues *pMountedKV = new KeyValues("CEconItemSchema");
	if (pMountedKV->LoadFromBuffer(NULL, bufText))
	{
		SCHEMA_INIT_SUBSTEP(BInitMountedScript(pMountedKV, pVecErrors));
	}

	return SCHEMA_INIT_SUCCESS();
}

bool CEconItemSchema::BInitMountedScript(KeyValues* pMountedKV, CUtlVector<CUtlString>* pVecErrors)
{
	// Parse the prefabs block first so the prefabs will be populated in case anything else wants
	// to use them later.
	KeyValues* pKVPrefabs = pMountedKV->FindKey("prefabs");
	if (NULL != pKVPrefabs)
	{
		SCHEMA_INIT_SUBSTEP(BInitDefinitionPrefabs(pKVPrefabs, pVecErrors));
	}
	...
	// Initialize the attributes block
	KeyValues* pKVAttributes = pMountedKV->FindKey("attributes");
	SCHEMA_INIT_CHECK(NULL != pKVAttributes, "Required key \"attributes\" missing.\n");
	...
	// Initialize the items block
	KeyValues* pKVItems = pMountedKV->FindKey("items");
	SCHEMA_INIT_CHECK(NULL != pKVItems, "Required key \"items\" missing.\n");
...
void CEconItemSchema::RegisterItemDefinitionPrefabs(CEconItemDefinition* pItemDef)
{
	Assert(pItemDef);

	KeyValues* pKVItem = pItemDef->m_pKVItem;
	Assert(pKVItem);

	const char* pszPrefabName = pKVItem->GetString("prefab", NULL);
	if (!pszPrefabName)
		return;

	// Split the prefabs of the items
	CUtlStringList vecPrefabs;
	Q_SplitString(pszPrefabName, " ", vecPrefabs);

	FOR_EACH_VEC_BACK(vecPrefabs, i)
	{
		CUtlVector<CEconItemDefinition*>* itemList = GetItemListForDefinitionPrefab(vecPrefabs[i]);
		Assert(itemList);

		// This prefab already has this item, not need
		// to add it twice.
		if (itemList->HasElement(pItemDef))
			continue;

		// Add the item to the list of items that use this prefab.
		itemList->AddToTail(pItemDef);
	}
}
