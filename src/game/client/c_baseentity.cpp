...
static float AdjustInterpolationAmount( C_BaseEntity *pEntity, float baseInterpolation )
{
	...
	// Staging: This fixes a bug with the Sentry gun appearing jittery at low interp settings.
	const float minInterpolationTime = 0.1;
	const float minInterpolation = TICK_INTERVAL * ( TIME_TO_TICKS( minInterpolationTime ) + 1 );

	if ( minInterpolation > baseInterpolation )
	{
		while ( pEntity )
		{
