...
extern ConVar tf_weapon_allow_inspect;
...
void CBackpackPanel::OpenContextMenu()
{
	...
	if ( vecSelectedItems.Count() == 1 )
	{
		...
		if ( pItem && pItem->IsValid() &&
			( FindAttribute_UnsafeBitwiseCast<attrib_value_t>( pItem, pAttrib_WeaponAllowInspect, &flInspect ) || 
			FindAttribute_UnsafeBitwiseCast<attrib_value_t>( pItem, pAttrib_CosmeticAllowInspect, &flInspect ) ||
			tf_weapon_allow_inspect.GetBool() ) )
		{
			if ( flInspect != 0 || tf_weapon_allow_inspect.GetBool() )
			{
