...
class CDODHudWeaponSelection;
class CCSHudWeaponSelection;
class CHL2HudWeaponSelection;
class CHudChat;
class CHudVote;
...
ClientModeShared::ClientModeShared()
{
	...
	m_pDODWeaponSelection = NULL;
	m_pCSWeaponSelection = NULL;
	m_pHL2WeaponSelection = NULL;
...
void ClientModeShared::Init()
{
	...
	m_pDODWeaponSelection = ( CBaseHudWeaponSelection * )GET_HUDELEMENT( CDODHudWeaponSelection );
	Assert( m_pDODWeaponSelection );

	m_pCSWeaponSelection = ( CBaseHudWeaponSelection * )GET_HUDELEMENT( CCSHudWeaponSelection );
	Assert( m_pCSWeaponSelection );

	m_pHL2WeaponSelection = ( CBaseHudWeaponSelection * )GET_HUDELEMENT( CHL2HudWeaponSelection );
	Assert( m_pHL2WeaponSelection );
...
int ClientModeShared::HudElementKeyInput( int down, ButtonCode_t keynum, const char *pszCurrentBinding )
{
	...
	if ( m_pDODWeaponSelection )
	{
		if ( !m_pDODWeaponSelection->KeyInput( down, keynum, pszCurrentBinding ) )
		{
			return 0;
		}
	}

	if ( m_pCSWeaponSelection )
	{
		if ( !m_pCSWeaponSelection->KeyInput( down, keynum, pszCurrentBinding ) )
		{
			return 0;
		}
	}

	if ( m_pHL2WeaponSelection )
	{
		if ( !m_pHL2WeaponSelection->KeyInput( down, keynum, pszCurrentBinding ) )
		{
			return 0;
		}
	}
...
{
#if defined( REPLAY_ENABLED )
	if ( m_pReplayReminderPanel && g_pReplay && g_pReplay->IsRecording() && !::input->IsSteamControllerActive() )
	{
