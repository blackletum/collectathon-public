...
void CDODMapOverview::InitTeamColorsAndIcons()
{
	...
	m_TeamColors[TF_TEAM_RED] = COLOR_DOD_GREEN;
	m_TeamIcons[TF_TEAM_RED] = AddIconTexture( "sprites/minimap_icons/aplayer" );
	m_CameraIcons[TF_TEAM_RED] = AddIconTexture( "sprites/minimap_icons/allies_camera" );
	

	m_TeamColors[TF_TEAM_BLUE] = COLOR_DOD_RED;
	m_TeamIcons[TF_TEAM_BLUE] = AddIconTexture( "sprites/minimap_icons/gplayer" );
	m_CameraIcons[TF_TEAM_BLUE] = AddIconTexture( "sprites/minimap_icons/axis_camera" );
...
bool CDODMapOverview::DrawCapturePoint( int iCP, MapObject_t *obj )
{
	...
	else
	{
		...
		if ( iCappingTeam != TEAM_UNASSIGNED )
		{
			...
			bool bSwipeLeft = ( iCappingTeam == TF_TEAM_BLUE ) ? true : false;
		...
		int iNumAllies = g_pObjectiveResource->GetNumPlayersInArea( iCP, TF_TEAM_RED );
		int iNumAxis = g_pObjectiveResource->GetNumPlayersInArea( iCP, TF_TEAM_BLUE );

		int iOwningTeam = g_pObjectiveResource->GetOwningTeam( iCP );
		if ( iCappingTeam == TEAM_UNASSIGNED )
		{
			if ( iNumAllies > 0 && iNumAxis == 0 && iOwningTeam != TF_TEAM_RED )
			{
				iCappingTeam = TF_TEAM_RED;
			}
			else if ( iNumAxis > 0 && iNumAllies == 0 && iOwningTeam != TF_TEAM_BLUE )
			{
