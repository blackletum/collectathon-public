...
//-----------------------------------------------------------------------------
class CDODHudScope : public vgui::Panel, public CHudElement
{
	DECLARE_CLASS_SIMPLE( CDODHudScope, vgui::Panel );

public:
			CDODHudScope( const char *pElementName );
...
DECLARE_HUDELEMENT_DEPTH( CDODHudScope, 100 );
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CDODHudScope::CDODHudScope( const char *pElementName ) : CHudElement(pElementName), BaseClass(NULL, "HudScope")
{
...
//-----------------------------------------------------------------------------
void CDODHudScope::ApplySchemeSettings( vgui::IScheme *scheme )
{
...
void CDODHudScope::Paint( void )
{
	...
	{
	case DOD_WEAPON_SPRING:
		piScopeTex = m_iScopeSpringfield;
		break;
	case DOD_WEAPON_K98_SCOPED:
	...
	if( pPlayer->GetFOV() < 90 &&
		pWeapon->GetPCWpnData().m_DODWeaponData.m_WeaponType == WPN_TYPE_SNIPER )
	{
		int screenWide, screenTall;
		GetHudSize(screenWide, screenTall);
		...
		if ( pWeapon->GetWeaponID() == DOD_WEAPON_SPRING )
		{
