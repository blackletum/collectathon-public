...
CON_COMMAND( changegame, "Choose a game" )
{
	if ( !gViewPortInterface )
		return;
	...
	if ( pPlayer && ( pPlayer->GetTeamNumber() != TEAM_UNASSIGNED ) )
	{
		gViewPortInterface->ShowPanel( PANEL_GAMEMENU, true );
	}
}
...
//		newpanel = new CTFMapOverview( this );
	...
	else if ( Q_strcmp( PANEL_GAMEMENU, szPanelName ) == 0 )
	{
		newpanel = new CTFGameMenu( this );	
	}
...
void TFViewport::CreateDefaultPanels( void )
{
	...
	AddNewPanel( CreatePanelByName( PANEL_GAMEMENU ), "PANEL_GAMEMENU" );
