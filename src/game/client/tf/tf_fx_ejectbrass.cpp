...
void TF_EjectBrassCallback( const CEffectData &data )
{
	...
	if ( !pWeaponInfo )
		return;
	if ( !pWeaponInfo->m_ExtraTFWeaponData.m_szBrassModel || !pWeaponInfo->m_ExtraTFWeaponData.m_szBrassModel[0] )
		return;
	...
	model_t *pModel = (model_t *)engine->LoadModel( pWeaponInfo->m_ExtraTFWeaponData.m_szBrassModel );
	if ( !pModel )
		return;
