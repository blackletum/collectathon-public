...
void TFExplosionCallback( const Vector &vecOrigin, const Vector &vecNormal, int iWeaponID, ClientEntityHandle_t hEntity, int nDefID, int nSound, int iCustomParticleIndex )
{
	...
	if ( pWeaponInfo )
	{
		...
		if ( iCustomParticleIndex == INVALID_STRING_INDEX )
		{
			if ( bIsWater )
			{
				if ( Q_strlen( pWeaponInfo->m_ExtraTFWeaponData.m_szExplosionWaterEffect ) > 0 )
				{
					pszEffect = pWeaponInfo->m_ExtraTFWeaponData.m_szExplosionWaterEffect;
				}
			}
			else
			{
				if ( bIsPlayer || bInAir )
				{
					if ( Q_strlen( pWeaponInfo->m_ExtraTFWeaponData.m_szExplosionPlayerEffect ) > 0 )
					{
						pszEffect = pWeaponInfo->m_ExtraTFWeaponData.m_szExplosionPlayerEffect;
					}
				}
				else
				{
					if ( Q_strlen( pWeaponInfo->m_ExtraTFWeaponData.m_szExplosionEffect ) > 0 )
					{
						pszEffect = pWeaponInfo->m_ExtraTFWeaponData.m_szExplosionEffect;
					}
				}
			}
		}
		...
		// Sound.
		if ( Q_strlen( pWeaponInfo->m_ExtraTFWeaponData.m_szExplosionSound ) > 0 )
		{
			pszSound = pWeaponInfo->m_ExtraTFWeaponData.m_szExplosionSound;
		}
	}
