...
void C_TFObjectiveResource::SetCappingTeam( int index, int team )
{
	...
	if ( GetNumControlPoints() > 0 && GetCapWarningLevel( index ) == CP_WARN_NORMAL && GetCPCapPercentage( index ) == 0.0f && team != TEAM_UNASSIGNED )
	{
		...
		if ( pLocalPlayer )
		{
			if ( pLocalPlayer->GetTeamNumber() != team )
			{
				...
				if ( GetOwningTeam( index ) != TEAM_UNASSIGNED )
				{
