...
void LoadHudTextures( CUtlDict< CHudTexture *, int >& list, const char *szFilenameWithoutExtension, const unsigned char *pICEKey, const char *szSearchPath )
{
	...
	while (true)
	{
		KeyValues* pKeyValuesData = ReadEncryptedKVFile(filesystem, szFilenameWithoutExtension, pICEKey, false, (bMount) ? ".mount" : ".txt", szSearchPath );
		if (pKeyValuesData)
		{
			...
			if (pTextureSection)
			{
				...
				while (pTemp)
				{
					if (pTemp->GetString("font", NULL))
					{
						...
						if (!list.HasElement(tex->szShortName))
...
#define LOADGAMEHUDTEXTURES( TEXTURELIST, GAMENAME )							\
	LoadHudTextures( TEXTURELIST, "scripts/hud_textures", NULL, GAMENAME );		\
	LoadHudTextures( TEXTURELIST, "scripts/mod_textures", NULL, GAMENAME );		\
...
#if defined( TF_CLIENT_DLL ) || defined( DOD_DLL ) || defined( TF_BASE )
...
void CHud::Init( void )
{
	...
	LOADGAMEHUDTEXTURES( textureList, "tfbase" );
	LOADGAMEHUDTEXTURES( textureList, "dod" );
	LOADGAMEHUDTEXTURES( textureList, "shared" );
	LOADGAMEHUDTEXTURES( textureList, "cstrike" );
	LOADGAMEHUDTEXTURES( textureList, "hl2_override" );
	LOADGAMEHUDTEXTURES( textureList, "hl1" );
