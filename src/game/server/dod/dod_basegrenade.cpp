...
void CDODBaseGrenade::Spawn( void )
{
	...
	AddFlag( FL_GRENADE ); // Grenade flag needed for airblast deflection
...
void CDODBaseGrenade::Precache( void )
{
	...
	PrecacheParticleSystem( "dod_explosioncore_midair" );
	PrecacheParticleSystem( "dod_explosioncore_floor" );
}
...
void CDODBaseGrenade::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	// Cancel out if we're not a dod class
	if ( !ToTFPlayer( pActivator )->IsDODClass() )
		return;
...
void CDODBaseGrenade::Detonate()
{
	...
	TFGameRules()->RadiusStun( info, GetAbsOrigin(), m_DmgRadius );
...
//-----------------------------------------------------------------------------
void CDODBaseGrenade::Deflected( CBaseEntity *pDeflectedBy, Vector& vecDir )
{
	...
	// SEALTODO Weapon id might be wrong here
	CTFWeaponBase::SendObjectDeflectedEvent( pTFDeflector, pOldOwner, GetEmitterWeaponID(), this );
