...
CBasePlayer *BotPutInServer( bool bTargetDummy, bool bFrozen, int iTeam, int iClass, const char *pszCustomName )
{
	...
	//pPlayer->SetPlayerType( CTFPlayer::TEMP_BOT ); // SEALTODO
...
CON_COMMAND_F( bot, "Add a bot.", FCVAR_NONE )
{
	...
	while ( --count >= 0 )
	{
		...
		int iClass;
		bool bIsAllowed = false; // Is this class blocked for whatever reason?

		// Don't allow bots to pick specical classes.
		do{
			iClass = g_pServerBenchmark->RandomInt( GAME_FIRST_CLASS, GAME_LAST_CLASS );
		} while ( bIsAllowed == ( GetPlayerClassData( iClass )->m_iClassType != FACTION_PC ) );

		char const *pVal = args.FindArg( "-class" );
		if ( pVal )
		{
			for ( int i=1; i < GAME_CLASS_COUNT; i++ )
			{
				if ( !Q_stricmp( pVal, g_aRawPlayerClassNamesShort[i] ) )
				{
					iClass = i;
					break;
				}
		...
		if ( player->GetFlags() & FL_FAKECLIENT )
		{
				 ...
				 ( player->GetTeamNumber() == iTeam ) )
			{
				...
				if ( bBurn )
				{
					...
					pBot->m_Shared.Burn( nullptr, 10.0f );
				}
			}
		}
	}
...
CON_COMMAND_F( bot_whack, "Deliver lethal damage from player to specified bot. Usage: bot_whack <bot name>", FCVAR_CHEAT )
{
	...
	FOR_EACH_VEC( botVector, i )
	{
		...
		info.SetInflictor( pTFPlayer->GetActivePCWeapon() );
...
void botdata_t::RunAttackPlan( void )
{
	...
	if ( bCanSeeTarget )
	{
		...
		if ( m_hBot->GetActivePCWeapon() && m_hBot->GetActivePCWeapon()->IsMeleeWeapon()  )
		{
