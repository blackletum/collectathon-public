...
bool CObjectDispenser::DispenseAmmo( CTFPlayer *pPlayer )
{
	...
	if ( nNoPrimaryAmmoFromDispensersWhileActive == 0 )
	{
		...
		iTotalPickedUp += pPlayer->GiveAmmo( iAmmoToAdd, TF_AMMO_PRIMARY, !m_bPlayAmmoPickupSound );
	}

	// secondary
	iAmmoToAdd = (int)( pPlayer->GetMaxAmmo( TF_AMMO_SECONDARY ) * flAmmoRate );
	iTotalPickedUp += pPlayer->GiveAmmo( iAmmoToAdd, TF_AMMO_SECONDARY, !m_bPlayAmmoPickupSound );
...
int CObjectDispenser::DispenseMetal( CTFPlayer *pPlayer )
{
	...
	int iMetal = pPlayer->GiveAmmo( Min( m_iAmmoMetal.Get(), iMetalToGive ), TF_AMMO_METAL, false );
	m_iAmmoMetal -= iMetal;
...
{
	int iMetal = pPlayer->GiveAmmo( MIN( m_iAmmoMetal, DISPENSER_DROP_METAL ), TF_AMMO_METAL, false );
	return iMetal;
}
