...
void CPlayerMove::RunCommand ( CBasePlayer *player, CUserCmd *ucmd, IMoveHelper *moveHelper )
{
	...
	*/
	...
	player->GetGameMovementType()->StartTrackPredictionErrors( player );
	...
	if ( !pVehicle )
	{
		VPROF( "player->GetGameMovementType()->ProcessMovement()" );
		Assert( player->GetGameMovementType() );
		player->GetGameMovementType()->ProcessMovement( player, g_pMoveData );
	}
	else
	{
	...
	player->GetGameMovementType()->FinishTrackPredictionErrors( player );
