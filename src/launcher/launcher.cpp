...
#define DEFAULT_TF_GAMEDIR	"tf"
#define DEFAULT_TFBASE_GAMEDIR	"tfbase"
...
const char *CSourceAppSystemGroup::DetermineDefaultMod()
{
	if ( !m_bEditMode )
	{   		 
		return CommandLine()->ParmValue( "-game", DEFAULT_TFBASE_GAMEDIR );
	}
...
const char *CSourceAppSystemGroup::DetermineDefaultGame()
{
	if ( !m_bEditMode )
	{
		return CommandLine()->ParmValue( "-defaultgamedir", DEFAULT_TFBASE_GAMEDIR );
	}
...
bool GrabSourceMutex()
{
	...
	const char *pchGameParam = CommandLine()->ParmValue( "-game", DEFAULT_TFBASE_GAMEDIR );
	CRC32_t gameCRC;
	CRC32_Init(&gameCRC);
