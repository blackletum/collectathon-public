...
static bool LoadThisDll( char *szDllFilename, bool bIsServerOnly )
{
	...
	if ( !Host_AllowLoadModule( szDllFilename, "BIN", true, bIsServerOnly ) )
	{
		// not supposed to load this but we will anyway
		Host_DisallowSecureServers();
		Host_AllowLoadModule( szDllFilename, "BIN", true, bIsServerOnly );
	}
	...
	if ((pDLL = g_pFileSystem->LoadModule(szDllFilename, "BIN", false)) == NULL)
	{
