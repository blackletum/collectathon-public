...
ConVar snd_timescale_affects_pitch( "snd_timescale_affects_pitch", "1", FCVAR_CHEAT );
...
void MIX_MixChannelsToPaintbuffer( CChannelList &list, int endtime, int flags, int rate, int outputRate )
{
	...
	if ( snd_timescale_affects_pitch.GetBool() && sv_cheats.GetBool() )
		flGlobalPitchScale = host_timescale.GetFloat();
